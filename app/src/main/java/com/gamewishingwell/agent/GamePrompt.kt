...
$seedSystems
    ...
    /**
     * 修改范围契约（硬性）：修改/修复回合注入——除非用户本轮明确要求，禁止改变既有
     * 游戏内容与逻辑，只允许最小侵入地实现用户点名的部分。执行层配套 EntryRewritePolicy
     * 拒绝对已有 index.html 的整量重写；提示词与执行层双保险。
     */
    const val SCOPE_FENCE_RULE =
        "【修改范围契约（硬性）】这是对已有游戏的修改回合：除非用户本轮明确要求，" +
            "禁止改变任何既有游戏内容与逻辑——数值、规则、放置/生成/判定逻辑、布局、" +
            "操作方式、关卡结构、音效、文案、命名等一律保持原样，不顺手优化或重构无关部分。" +
            "只改动与用户诉求直接相关的代码，以最小侵入方式实现：不重命名既有变量与函数、" +
            "不调整无关常量。确需结构性大改（如整体重写）时不要自行实施——在交付总结中" +
            "向用户说明原因并等待明确同意。"
    ...
    fun toolWorkflowPrompt(firstGeneration: Boolean): String {
        ...
        2. 修改一律用 editfile 精确替换（常规修改首选）或 appendfile 追加新代码段；需要改动一大段时，old_string 取该段完整原文（如整个函数块）做整段替换；
  ...
  - 同一处反复修复失败时，换实现思路（重构该函数 / 换数据结构），用 editfile 整段替换问题模块（old_string 取该模块完整原文）。
  - 不要在回复正文里粘贴整个文件的代码——文件内容只通过工具写入。
  - 修改已有游戏时禁止"顺手"改动用户未要求的部分：只改与诉求直接相关的代码，数值、音效、布局、命名等未点名内容一律保持原样（修改回合对已有 index.html 的整量重写会被执行层直接拒绝；首次生成回合拦一次后放行）。
