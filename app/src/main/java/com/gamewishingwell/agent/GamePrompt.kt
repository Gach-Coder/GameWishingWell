...
object GamePrompt {
    ...
    private fun hardRules(): String = """
        ...
        5. 事件处理与主循环内部可用 try/catch 防止单点异常中断全局，但 catch 内必须 console.error(e) 上报——
           沙箱靠 console 采集发现真实错误，静默吞错（空 catch / 只 return 的 catch）会把可修 bug 变成不可见的画面/逻辑故障。
        6. 二维数组访问必须安全：先取行再判空，绝不能直接 grid[i][j]。
        7. 绘制函数必须在数据初始化完成前安全返回；init 里先做数据初始化再做绘制。
        8. 遇到 NaN/undefined 类错误必须溯源修复：检查顶点/数据数组的结构与索引方式是否匹配
           （嵌套顶点表配扁平索引、undefined 键取值是最常见根因）、字段是否存在。禁止用
           "过滤坏值（isFinite 筛选丢弃）/默认值兜底（a||默认）"消除报错后交付——那只是掩盖症状，
           数据仍是坏的，画面不会正确。
...
$seedSystems
    ...
    private fun scenarioContract(): String = """
        ...
        - 回归守恒（执行层强制）：修改回合的断言只增不减——既有断言 id 一个都不能少；修断言内容（对照失败轨迹对齐 expect/steps）保留 id 原位改写；玩家报障修复把问题固化为新断言追加；删除或合并条目需用户明确授权（重做游戏/精简断言）。
        - 套件上限 ${GameScenarios.MAX_SCENARIOS} 条：超限时需用户授权精简（合并同类或替换最弱断言，优先保留核心闭环与已修复缺陷条目）。
        - 沙箱交付验收会逐条确定性执行；未通过以 scenario-fail 回传，必须修复游戏逻辑或对齐断言字段后重跑。
        - 迭代/修复轮维护此文件：玩家报障修复后把该问题固化为新断言追加；整体重写断言套件仅当用户明确授权重做（与写入铁律同源——擅自重写等于给回归保护调包）。
    """.trimIndent()
    ...
    fun engineContract(
        ...
        if (GameEngines.PIXI in allowed) {
            ...
            new PIXI.Application({view:canvas, antialias:true, backgroundAlpha:0, preserveDrawingBuffer:true})
            （沙箱画面检测需读回 canvas 像素，缺少 preserveDrawingBuffer 会被判纯色画布回炉）；精灵纹理用离屏 Canvas 程序化生成
    ...
    fun toolWorkflowPrompt(firstGeneration: Boolean): String {
        ...
        2. 修改用 editfile 精确替换（定位到相关函数/数据表逐段替换）或 appendfile 追加；改动跨文件时对每个文件分别 editfile（同轮并行发起）；
           【写入铁律（执行层强制，违反=致命错误直接拒绝）】未经用户授权（明确说重做/重构），禁止整体重写已有游戏文件——
           执行层把每次写入结果与回合起点比对行相似度：无论 writefile 一次达成还是多次 editfile 逐步替换，
           相似度过低一律拦下；新建文件、局部修改、追加不受限。scenarios.json 可整体重写落盘（JSON 的自然编辑方式），
           但回归断言只增不减（删 id 同样致命）。新增系统优先新建独立文件（js/<系统>.js）＋在 index.html 补引用。
  ...
  - 不要在回复正文里粘贴整个文件的代码——文件内容只通过工具写入。
        ...
        - 写入铁律（执行层强制）：未经用户授权（重做/重构）禁止整体重写已有游戏文件——执行层按回合起点比对行相似度，多次小替换逐步掏空同样会被拦；新建、局部修改、追加不受限。
