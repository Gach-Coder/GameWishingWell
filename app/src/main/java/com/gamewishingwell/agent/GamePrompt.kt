...
object GamePrompt {
    ...
    private fun baseSystemPrompt(toolMode: Boolean): String = """
        ...
        3. 必须适配手机触控：不依赖键盘或鼠标；用 touchstart / touchmove / touchend 实现虚拟摇杆、点按或滑动操作，并 preventDefault 阻止页面滚动；canvas 随窗口尺寸自适应。
           ...
           - 横板游戏：平台保证以真实横屏视口（宽>高）加载，直接以窗口实际尺寸为逻辑分辨率（如 W=innerWidth、H=innerHeight）全屏铺满，布局用相对比例（地面高度、跳跃高度等随 H 校准），HUD 与控件从顶部约 120px 安全区之下开始。
...
$seedSystems
    ...
    fun playabilityRequirements(): String = """
        ...
        5. 触控体验：操作目标足够大（≥44px）、位置顺手不遮挡视线；点按/拖动响应即时；横板动作类按标准布局（左下 ◀▶ 方向键、右下跳跃键，方向语义为触点横向位置）；顶部约 110px 平台保留区整条禁止按钮与可交互元素，左右两侧不放任何文字标签，仅中央小块可放“第x关”类极短状态文字（沙箱会自动检测该区域内的可交互元素）。
    ...
    fun codeContract(): String = """
        ...
        输出前确认：无 eval、无外部资源、restart() 存在、__wwDebugState 快照存在、触控可玩、声明前置、主画布方向与 design_schema 的 orientation 一致且铺满视口（无固定分辨率黑边）、触控控件为标准布局（横板左下◀▶右下跳跃，方向语义为横向）、顶部保留区无按钮且左右无文字标签。
    """.trimIndent()
